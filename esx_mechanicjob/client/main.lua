...
function OpenMechanicActionsMenu()
	...
	--	{label = "Вземи repair kit",  value = 'get_kit'},
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		elseif data.current.value == 'cloakroom' then
			menu.close()
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)------------------- SET CLOTHES
		if skin.sex == 0 then
		   skinche = {
        ...
        end     if skin.sex == 0 then
					TriggerEvent('skinchanger:loadClothes', skin, skinche)
				else
					TriggerEvent('skinchanger:loadClothes', skin, skinche)
				end
			end)
		...
		elseif data.current.value == 'get_kit' then
			TriggerServerEvent('esx_mechanicjob:get_kit')
...
AddEventHandler('esx_mechanicjob:hasEnteredMarker', function(zone)
	...
	elseif zone == 'BossActions' then
	    CurrentAction     = 'boss_actions'
		CurrentActionMsg  = "Натисни ~INPUT_CONTEXT~ за да отвориш шеф менюто"
		CurrentActionData = {}
